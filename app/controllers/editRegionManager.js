...
(function () {
  ...
  var editRegionManagerController = function ($scope,$rootScope,$location,$filter,editRegionManagerService) {
    ...
    $scope.updateRegionManager=function(rm_id){
      ...
      $scope.rm_id=rm_id;
      $scope.rm_mob=$scope.regionvalue1.rm_mob;
      $scope.rm_alt_mob_no=$scope.regionvalue1.rm_alt_mob_no;
      $scope.rm_email=$scope.regionvalue1.rm_email;
      $scope.rm_address=$scope.regionvalue1.rm_address.split(',')[0] + "," + $scope.regionvalue1.rm_address.split(',')[1] ;
      $scope.rm_dob=$scope.dob;
      $scope.rm_gender=$scope.regionvalue1.rm_gender;
      ...
      $scope.rm_education_3=$scope.education3degree + "," + $scope.education3year + "," + $scope.education3Uni;
       ...
       for( var i=0;i<3;i++){
        ...
        { 
          if(key=="Hindi"){
          $scope.language1=key;
          $scope.rmlanguage1=$scope.language1;
          $scope.rm_language=$scope.rmlanguage1;
          }
        
          if(key == "English"){
            $scope.language2=key;
            $scope.regManager2 = $scope.language2;
            $scope.rm_language = $scope.regManager2;

           
          }
          if($scope.rmlanguage1 && $scope.regManager2 ){
            $scope.bothlang=$scope.rmlanguage1 +","+ $scope.regManager2;
            $scope.rm_language =$scope.bothlang;
       ...
       editRegionManagerService.updateRegionManager($scope.rm_name,$scope.rm_id,$scope.rm_mob,$scope.rm_alt_mob_no,$scope.rm_email,$scope.rm_address,$scope.rm_dob,$scope.rm_gender,$scope.rm_education_1,$scope.rm_education_2,$scope.rm_education_3).success(function (result) {
